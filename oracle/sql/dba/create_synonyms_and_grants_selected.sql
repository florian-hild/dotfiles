...
SELECT '$HOME/create_synonyms_and_grants_selected_' || to_char(sysdate, 'yyyy-mm-dd_hh24-mi-ss') || '.sql' spoolname FROM dual;
SPOOL '&spoolname'
...
DECLARE
  ...
  IS
    SELECT t.OBJECT_NAME, t.OBJECT_TYPE
      ...
      AND NOT EXISTS ( SELECT 1 FROM all_synonyms s WHERE s.owner = UPPER('&dstuser') AND s.table_owner = user AND s.table_name = t.OBJECT_NAME )
