...
-- Author     : Florian Hild
-- Created    : 24-05-2024
-- Description: Generate disable and enable scripts for all enabled triggers
--              of the current user (no shell commands needed).
--------------------------------------------------------------------------------
...
BEGIN
  FOR trigger_record IN select_triggers
  LOOP
    dbms_output.put_line ('alter trigger ' || user || '.' || trigger_record.trigger_name || ' enable;');
  END LOOP;
END;
/
